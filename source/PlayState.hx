...
class PlayState extends MusicBeatState
{
	...
	var sarv:Character;
	var zardy:Character;
	var garcello:Character;
	var garcelloleft:Character;
	var whittyright:Character;
	var garcelloghost:Character;
	var zardyright:Character;

	var sarvr:Character;
	var whittyr:Character;
	...
	var isgarcello:Bool = false;
	var isgarcelloleft:Bool = false;
	var isgarcelloghost:Bool = false;
	var iszardyright:Bool = false;
	var iswhittyfade:Bool = false;
	var issarvfade:Bool = false;
	var iswhittyright:Bool = false;
	...
	override public function create()
	{
				...
				case 'gar':
					{
						defaultCamZoom = 0.7;
								  ...
								  smoker = new FlxSprite(0, -400);
								  smoker.frames = Paths.getSparrowAtlas('garSmoke');
								  smoker.setGraphicSize(Std.int(smoker.width * 2.7));
								  smoker.alpha = 0.3;
								  ...
								  sarvbg = new FlxSprite(-347, -800).loadGraphic(Paths.image('sarv/base'));
								  ...
								  whitbg = new FlxSprite(-500, -170).loadGraphic(Paths.image('sarv/whitbg'));
								  whitbg.antialiasing = true;
								  whitbg.scrollFactor.set(0.9, 0.9);
								  whitbg.active = false;
								  whitbg.alpha = 0;
								  add(whitbg);

								  zarbg = new FlxSprite(-500, -170).loadGraphic(Paths.image('sarv/zarbg'));
		...
		sarv = new Character(-200, 200, 'sarv');
		garcello = new Character(900, 200, 'garcellodead');
		whitty = new Character(-300, 200, 'whitty');
		zardy = new Character(200, 200, 'zardy');
		//Fading
		garcelloghost = new Character(800, 200, 'garcelloghosty');
		whittyr = new Character(-200, 200, 'whitty');
		sarvr = new Character(-350, 200, 'sarv');
		zardyright = new Character(900, 200, 'zardyright');
		whittyright = new Character(900, 200, 'whittyright');
		garcelloleft = new Character(100, 100, 'garcelloleft');
			...
			if (curStage == 'gar')
				{
					...
					smoke = new FlxSprite(0, -400);
					smoke.frames = Paths.getSparrowAtlas('garSmoke');
					smoke.setGraphicSize(Std.int(smoke.width * 2.6));
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if (isgarcelloghost)
							{
								garcelloghost.playAnim('sing' + dadsDir[daNote.noteData], true);
								garcelloghost.holdTimer = 0;
							}
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						case "fading":
							    fancyOpenURL("https://www.youtube.com/watch?v=cRV4pgYxeiU");
				...
				else
				{
					...
					switch(SONG.song.toLowerCase())
                    {
						...
						case 'fading':
							LoadingState.loadAndSwitchState(new VideoState("assets/videos/cut4.webm",new PlayState()));
                        default:
	...
	*/
		...
		function goodNoteHit(note:Note, resetMashViolation = true):Void
			{
				...
				if (!note.wasGoodHit)
				{
					...
					if (iswhittyfade)
						{
							whittyr.playAnim('sing' + sDir[note.noteData], true);
							whittyr.holdTimer = 0;
						}
					if (issarvfade)
						{
							sarvr.playAnim('sing' + sDir[note.noteData], true);
							sarvr.holdTimer = 0;
						}
					if (iszardyright)
						{
							zardyright.playAnim('sing' + sDir[note.noteData], true);
							zardyright.holdTimer = 0;
						}
					if (iswhittyright)
						{
							whittyright.playAnim('sing' + sDir[note.noteData], true);
							whittyright.holdTimer = 0;
						}
	...
	override function stepHit()
	{
		...
		if (curStage == 'gar' && curStep != stepOfLast)
			{
				switch(curStep)
				{
					case 1:
				    ...
				    case 240:
					    ...
					    switchCharacter('gar');
						//switchCharacter('dad');
						iconP1.animation.play('garcellodead');
					case 272:
						resetCharacters();
						switchCharacter('dad');
						switchCharacter('bf');
						iconP1.animation.play('bf');
						iconP2.animation.play('sarv');
					case 304:
						resetCharacters();
						switchCharacter('bf');
					case 316:
					...
					case 784:
						...
						iconP1.animation.play('garcellodead');
					case 839:
						garcello.playAnim('garTightBars');
					//case 846:
		...
		if (curStage == 'fade' && curStep != stepOfLast)
			{
				switch(curStep)
				{
					case 1:
					...
					case 110:
						resetCharacters();
						switchCharacter('sarvf');	
						iconP1.animation.play('sarv');
					case 125:
						resetCharacters();
						switchCharacter('zardy');	
						iconP1.animation.play('zardy');
					case 141:
						resetCharacters();
						switchCharacter('bf');
                        switchCharacter('whittyr');
						switchCharacter('dad');
		...
		if (curStage == 'gar1' && curStep != stepOfLast)
			{
				switch(curStep)
				{
					case 1:
					...
					case 384:
						...
						switchCharacter('bf');
						iconP1.animation.play('bf');
						iconP2.animation.play('garcellodead');
					case 448:
						resetCharacters();
						switchCharacter('zardy');	
						switchCharacter('bf');
		...
		if (curStage == 'gar2' && curStep != stepOfLast)
			{
				switch(curStep)
				{
					case 1:
